<template>
	<view class="content">
		...
		<view class="main" v-for="(item,index) in list" :key="index" :id="item.uid" @click="change">
			...
			<view class="set">
				<text @click.stop="setFn" :id="item.uid">设置利率</text>
			</view>
			<view class="main_flex_between">
				<view class="main_item">
					下级人数:{{item.subnum}}
...
<script>
	export default {
		...
		methods: {
			...
			setFn(e) {
				uni.navigateTo({
					url: "setSub?id=" + e.currentTarget.id
				})
			},
			find() {
				this.page = 1;
			...
			change(e) {
				uni.navigateTo({
					url: 'subordinateOne?id=' + e.currentTarget.id
				})
			},
		}
	}
